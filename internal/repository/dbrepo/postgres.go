...
// func (m *postgresDBRepo) GetUserByEmailAddress(email string) (models.User, error) {
// 	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
// 	defer cancel()
...
// updates procces for resevation
...
// returns restriction for a room by dates
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	row, err := m.DB.QueryContext(ctx, query, start, end, roomID)
	if err != nil {
...
// deletes a block for by using date
func (m *postgresDBRepo) DeleteBlockedRoom(restrictionID int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) InsertBlockOnRoom(roomID int, start time.Time) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
